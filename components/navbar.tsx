  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 pt-[env(safe-area-inset-top)] transition-colors duration-300 ${
        scrolled || open ? "bg-[#111111]" : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between gap-2 px-4 sm:h-20 sm:gap-3 sm:px-8 lg:h-[110px] lg:px-10">
        <Link
          href="/"
          ...
          <nav className="flex items-center gap-8 xl:gap-10" aria-label="Primary">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`font-display text-lg font-extrabold uppercase tracking-wide transition-colors ${
        ...
        <div className="flex items-center gap-2 sm:gap-3 lg:hidden">
          <Link
            href="/contact"
            className="inline-flex min-h-10 items-center justify-center bg-[#F7EB4F] px-3 py-2 font-display text-xs font-extrabold uppercase tracking-wide text-[#111111] sm:min-h-11 sm:px-4 sm:text-sm"
          >
            Get a Quote
          </Link>
          <button
      ...
      >
        <nav
          className={`flex h-full flex-col justify-center gap-1 px-6 pt-[calc(5.5rem+env(safe-area-inset-top))] pb-[max(1.5rem,env(safe-area-inset-bottom))] transition-transform duration-300 sm:px-8 ${
